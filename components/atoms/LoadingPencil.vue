<template>
	<div class="rainbow-marker">
		<h1 class="rainbow-marker__title">{{ loadingText }}</h1>
		<div class="rainbow-marker__loader"></div>
	</div>
</template>
...
const loadingText = ref(t('atoms.loading'))
const setIntervalId = ref(null)

const animateLoading = () => {
	setInterval(() => {
		if (loadingText.value.length >= 7 && loadingText.value.length < 10) {
			loadingText.value += '.'
		} else {
			loadingText.value = loadingText.value.slice(0, 7)
		}
	}, 500)
}

onMounted(() => animateLoading())
